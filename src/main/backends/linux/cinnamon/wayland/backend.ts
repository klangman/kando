...
// SPDX-FileCopyrightText: Kevin Langman <klangman@gmail.com>// SPDX-FileCopyrightText: Kevin Langman <klangman@gmail.com>
...
/**
 * This backend uses the DBus interface of the Kando Cinnamon integration
 * extension to interact with the system. As such, it only works on Cinnamon
 * with the Kando extension installed. It would also work on X11, but the generic
 * X11 backend is preferred on X11 as it does not require any extensions.
 */
export class CinnamonWaylandBackend implements Backend {
  /** This maps GDK shortcut strings to the registered shortcuts. */
  private callbacks: { [shortcut: string]: () => void } = {};

  /** This is the DBus interface of the Kando Cinnamon integration extension. */
  private interface: DBus.ClientInterface;

  /**
   * On Cinnamon, we use a dock window. This creates a floating window which is
   * always on top of all other windows. It even stays visible during workspace
   * switching.
   */
  public getBackendInfo() {
    return {
      name: 'Cinnamon Wayland Backend',
      windowType: 'dock',
  ...
  /**
   * Initializes the backend. It connects to the DBus interface of the Kando Cinnamon
  ...
  public async init() {
    ...
    try {
      ...
      const obj = await bus.getProxyObject(
        'org.Cinnamon',
        '/org/cinnamon/extensions/KandoIntegration'
      );

      this.interface = obj.getInterface('org.Cinnamon.Extensions.KandoIntegration');
      ...
      throw new Error(
        i18next.t('backends.cinnamon.error', {
          link: 'https://cinnamon-spices.linuxmint.com/',
        })
      );
    }
  }
  ...
  /**
   * This uses the DBus interface of the Kando Cinnamon integration extension to get
  ...
  /**
   * Simulates a sequence of key presses using the Cinnamon extension. If one of the
   ...
   * shortcut is pressed. On Cinnamon Wayland, this uses the DBus interface of the
   * Kando Cinnamon integration.
   *
  ...
  private toGdkShortcut(shortcut: string) {
    if (shortcut.includes('Option')) {
      throw new Error('Shortcuts including Option are not yet supported on Cinnamon.');
    }

    if (shortcut.includes('AltGr')) {
      throw new Error('Shortcuts including AltGr are not yet supported on Cinnamon.');
    }
